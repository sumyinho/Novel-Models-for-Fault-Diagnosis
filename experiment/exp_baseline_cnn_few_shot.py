...
from util.utils import evaluate_model, get_k_samples
...
# root_dict = {"../dataset/XJTU-SY_Bearing_Datasets/": "XJTU"}
...
for root, dataset_name in root_dict.items():
    ...
    for i in np.arange(10):
        train_samples = get_k_samples(samples, k=10)
        train_data = train_samples[:, :-1]
        train_label = train_samples[:, -1]

        train_dataset = VibrationSignalDataset(train_data, train_label)
        test_dataset = VibrationSignalDataset(samples[:, :-1], samples[:, -1])
        train_dataloader = DataLoader(train_dataset, batch_size=1)
        test_dataloader = DataLoader(test_dataset, batch_size=batch_size)
        test_data = torch.FloatTensor(np.expand_dims(samples[:, :-1], axis=1))
        test_labels = torch.LongTensor(samples[:, -1])
        loss_list = []
