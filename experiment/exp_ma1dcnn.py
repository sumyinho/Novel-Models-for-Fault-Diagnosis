...
from model.ma1dcnn import Ma1dCNN
from util.dataset import VibrationSignalDataset
from util.utils import evaluate_model, weights_init
...
for root, dataset_name in root_dict.items():
    ...
    for train_index, test_index in k_flod_idx:
        ...
        model = Ma1dCNN(number_of_class=number_of_class)
        model.apply(weights_init)  # init model
        model = model.to(device)
        model_best = model
        accuracy_best = 0
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
        scheduler = ReduceLROnPlateau(optimizer, mode='min', factor=0.9, patience=5, verbose=False,
