...
from model.rsbu import DeepResidualShrinkageNetwork
...
for train_index, test_index in k_flod_idx:
    ...
    model = DeepResidualShrinkageNetwork(number_of_class=number_of_class)
    model = model.to(device)
    model_best = model
    accuracy_best = 0
    optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate, momentum=0.9, weight_decay=1e-4)
    schedule = MultiStepLR(optimizer, milestones=[40, 180], gamma=0.1)
...
with open("../experiment_data/rsbu_PU.pkl", "wb") as f:
