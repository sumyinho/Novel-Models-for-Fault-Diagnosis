...
class DepthwiseConv1D(nn.Module):
    def __init__(self, dim_in, kernel_size, dilation_rate, depth_multiplier, padding="same",
                 use_bias=False):
        super().__init__()
        self.conv = nn.Conv1d(in_channels=dim_in, out_channels=dim_in * depth_multiplier, kernel_size=kernel_size,
                              stride=1, padding=padding, groups=dim_in,
...
class Mixconv(nn.Module):
    def __init__(self, channal=64, kersize=64, m=1, c=1, dim_in=128):
        ...
        self.depth_conv_1 = DepthwiseConv1D(dim_in=dim_in, kernel_size=kersize, dilation_rate=m, depth_multiplier=c,
                                            padding="same",
                                            use_bias=False)
        self.act_2 = nn.ReLU()
...
class MIXCNN(nn.Module):
    def __init__(self, number_of_class):
        ...
        self.fc = nn.Linear(128, number_of_class)
